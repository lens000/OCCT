...
public:
    ...
    // 并行文件去重
    void removeDuplicatesParallel() {
        std::cout << "开始并行文件去重..." << std::endl;
        ...
        // 并行去重
        auto last = tbb::parallel_reduce(
            tbb::blocked_range<size_t>(1, lines.size()),
            lines.begin(),
            [&](const tbb::blocked_range<size_t>& range, std::vector<std::string>::iterator init) {
                auto result = init;
            ...
            [](std::vector<std::string>::iterator a, std::vector<std::string>::iterator b) {
                return std::max(a, b);
            }
        );
        ...
        std::cout << "去重完成，从 " << original_size << " 行减少到 " << lines.size() << " 行" << std::endl;
    ...
    // 并行文件转换
    void transformFileParallel() {
        std::cout << "开始并行文件转换..." << std::endl;
        ...
        std::cout << "文件转换完成，耗时: " << duration.count() << " ms" << std::endl;
...
int main() {
    ...
    // 测试并行去重
    std::cout << "\n3. 测试并行去重..." << std::endl;
    processor.removeDuplicatesParallel();
    ...
    // 测试并行文件转换
    std::cout << "\n5. 测试并行文件转换..." << std::endl;
    processor.transformFileParallel();
